...
import { Check, Copy, ExternalLink, Facebook, Printer, RotateCcw, Share2, ThumbsDown, ThumbsUp, Twitter, X, Zap } from 'lucide-react';
...
function LoadingResults({ step }: { step: 1 | 2 }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex min-h-[60vh] items-center justify-center py-16">
...
function EmptySearch() {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 min-h-[60vh] py-16">
...
function Results({ result }: { result: RankFinalResult }) {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6 py-8 print:max-w-none print:p-0">
      <section className="filters rounded-card border border-border bg-surface p-4 shadow-surface">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="secondary" onClick={() => typeof window !== 'undefined' && window.print()}>
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return <ErrorState onRetry={() => typeof window !== 'undefined' && window.location.reload()} />;
    }
...
function SearchPageContentWrapper() {
  return (
    <Suspense fallback={<LoadingResults step={1} />}>
      <SearchPageContent />
    </Suspense>
  );
}
