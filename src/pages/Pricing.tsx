...
const faqs = [
  ...
  ["What payment methods do you accept?", "RankFinal accepts major cards and standard Euro billing options. Business customers can request invoice billing."],
...
function FeatureRow({ children, included }: { children: React.ReactNode; included: boolean }) {
  return (
    <li className="flex gap-3 text-sm leading-6">
      {included ? <Check className="mt-1 size-4 shrink-0 text-success" /> : <X className="mt-1 size-4 shrink-0 text-destructive" />}
      <span className={included ? "text-text-primary" : "text-text-secondary"}>{children}</span>
    </li>
  );
}
    ...
    <PageWrapper className="space-y-16 py-10 lg:py-16">
      <section className="mx-auto max-w-4xl space-y-6 text-center">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl font-extrabold tracking-tight text-text-primary sm:text-6xl">Choose the plan that fits your needs</h1>
          <p className="text-lg font-medium text-text-secondary">All prices in Euro. Cancel anytime. No hidden fees.</p>
        </div>
        <div className="mx-auto inline-flex items-center rounded-pill border border-border bg-surface p-1 shadow-surface">
          <button type="button" onClick={() => setAnnual(false)} className={cn("rounded-pill px-5 py-2 text-sm font-bold transition-all", !annual ? "bg-accent-amber text-primary-foreground" : "text-text-secondary hover:text-text-primary")}>Monthly</button>
      ...
      <section className="grid gap-5 lg:grid-cols-3" aria-label="Pricing plans">
        {plans.map((plan) => (
          <Card key={plan.name} className={cn("relative flex flex-col", plan.name === "Free" && "border-accent-purple", plan.featured && "border-accent-amber shadow-amber lg:-translate-y-3")}>
            {plan.featured && <Badge variant="amber" className="absolute right-5 top-5">Most popular</Badge>}
            {plan.name === "Free" && <Badge variant="gray" className="absolute right-5 top-5">Current plan</Badge>}
            <CardHeader className="space-y-4">
              <CardTitle className="text-2xl">{plan.name}</CardTitle>
              <div>
                <span className="text-5xl font-extrabold text-text-primary">{annual ? plan.annual : plan.monthly}</span>
                <span className="ml-1 text-sm font-semibold text-text-secondary">{annual && plan.name !== "Free" ? plan.suffix : "/month"}</span>
              </div>
            </CardHeader>
      ...
      <section className="space-y-6" aria-labelledby="comparison">
        <div className="space-y-2 text-center">
          ...
          <h2 id="comparison" className="text-3xl font-extrabold text-text-primary">Full feature matrix</h2>
        </div>
        <Card>
              ...
              <tbody>
                {matrix.map(([feature, free, pro, business]) => (
                  <tr key={String(feature)} className="border-b border-border">
                    <td className="p-4 font-bold text-text-primary">{feature}</td>
                    <td className="p-4 text-center text-text-secondary"><MatrixValue value={free} /></td>
                    <td className="p-4 text-center text-text-secondary"><MatrixValue value={pro} /></td>
                    <td className="p-4 text-center text-text-secondary"><MatrixValue value={business} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mx-auto max-w-3xl space-y-5" aria-labelledby="faq">
        <div className="text-center">
          <Badge variant="gray">FAQ</Badge>
          <h2 id="faq" className="mt-4 text-3xl font-extrabold text-text-primary">Questions before you choose?</h2>
        </div>
        <div className="space-y-3">
          {faqs.map(([question, answer], index) => (
            <Card key={question} className="hover:shadow-surface">
              <button type="button" onClick={() => setOpenFaq(openFaq === index ? -1 : index)} className="flex w-full items-center justify-between gap-4 p-5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <span className="font-extrabold text-text-primary">{question}</span>
                <ChevronDown className={cn("size-5 shrink-0 text-accent-amber transition-transform", openFaq === index && "rotate-180")} />
              </button>
              {openFaq === index && <div className="px-5 pb-5 text-sm leading-7 text-text-secondary animate-fade-in">{answer}</div>}
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-card border border-accent-amber/60 bg-surface p-6 text-center shadow-amber sm:p-10">
        <div className="mx-auto max-w-2xl space-y-5">
          <h2 className="text-3xl font-extrabold text-text-primary">Still not sure? Try Pro free for 14 days.</h2>
          <Button variant="amber" size="lg">Start free trial</Button>
        </div>
      </section>
    </PageWrapper>
  );
