      ...
      <nav className="mx-auto flex h-16 w-full max-w-[1280px] items-center gap-4 px-4 sm:px-6 lg:px-8" aria-label="Primary navigation">
        ...
        <div className="hidden items-center gap-5 lg:flex">
          ...
          <Button asChild variant="amber" size="pill">
            <Link to="/pricing">Get Pro <ArrowRight className="size-4" aria-hidden="true" /></Link>
          </Button>
        </div>
        ...
        <div className="fixed inset-x-0 top-16 z-50 min-h-[calc(100vh-4rem)] border-t border-border bg-background px-4 py-6 animate-slide-in-menu lg:hidden">
          <div className="mx-auto flex max-w-[1280px] flex-col gap-6">
            ...
            <div className="flex flex-col gap-1">
              {[...navItems, { label: "Help", href: "/help" }, { label: "Sign In", href: "/about" }]
                .filter((item, index, array) => array.findIndex((candidate) => candidate.href === item.href) === index)
                .map((item) => (
                  <NavLink key={item.href} to={item.href} className="rounded-input px-3 py-3 text-lg font-semibold text-text-primary hover:bg-surface" onClick={() => setOpen(false)}>
                    {item.label}
                  </NavLink>
                ))}
            </div>
            <Button asChild variant="amber" size="lg">
              <Link to="/pricing" onClick={() => setOpen(false)}>Get Pro <ArrowRight className="size-4" aria-hidden="true" /></Link>
            </Button>
          </div>
        </div>
      )}
